  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/75 backdrop-blur-md animate-in fade-in duration-200">
      ...
      <div className="fixed inset-y-0 right-0 flex max-w-full pl-10">
        <div className={`w-screen max-w-md shadow-2xl border-l flex flex-col justify-between animate-in slide-in-from-right duration-300 ${
          ...
          <div className="p-6 border-b border-zinc-800 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-zinc-800 text-xs font-mono text-zinc-300">
                {cart.length}
              </span>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {cart.length > 0 ? (
              cart.map((item) => (
                ...
                >
                  {/* Vector Image */}
                  <div className="size-20 shrink-0 rounded-xl bg-zinc-950 p-2 border border-zinc-800 flex items-center justify-center">
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <p className="text-[10px] font-mono text-zinc-400">${item.product.price} USD</p>

                    {/* Quantity controls */}
                    <div className="flex items-center gap-2 pt-1">
                      <button
                        onClick={() => updateQuantity(item.product.id, item.quantity - 1)}
                        className="size-6 rounded-full bg-zinc-800 text-white font-mono text-xs hover:bg-zinc-700 flex items-center justify-center"
                      >
                        -
                      </button>
              ...
              <div className="py-16 text-center space-y-3">
                <ShoppingBag className="size-12 mx-auto text-zinc-600" />
                <h3 className="text-base font-bold">Your Cart is Empty</h3>
                <p className="text-xs text-zinc-400">Explore the hardware catalog to add items.</p>
              </div>
            )}
          </div>
            ...
            <div className="p-6 border-t border-zinc-800 space-y-4">
              ...
              <form onSubmit={handleApplyPromo} className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="secondary" size="sm" type="submit" className="rounded-full text-xs px-3">
                    Apply
                  </Button>
                </div>
                {promoMessage && (
                  <p className={`text-[10px] ${promoMessage.error ? "text-red-400" : "text-emerald-400"}`}>
                    {promoMessage.text}
                  </p>
                )}
              </form>
              ...
              <div className="space-y-1.5 text-xs">
                ...
                <div className="flex items-center justify-between font-bold text-sm text-white pt-2 border-t border-zinc-800">
